define(['app', 'directive/ng-file'], function (app) {
    ...
    app.controller('softCategoryCtrl', ['$scope', '$http', function ($scope, $http) {
        $http.get('i/soft_categories').success(function (data) {
            $scope.data = data;
            if (data && data.length) {
                $scope.cg1 = data[0];
            }
        ...
        $scope.delete = function (c) {
            if (c.id) {
                if (confirm('删除类型将导致该类型下所有的文档处于无类型状态，确认删除吗？')) {
                    $http.delete('admin/softCategory/' + c.id).success(function (data) {
                        if (c.fid) {
        ...
        $scope.addModerator = function () {
            if ($scope.c && !$scope.c.fid) {
                ...
                if (/^1\d{10}$/.test(mobile)) {
                    $.post('admin/softCategory/' + $scope.c.id + '/moderators', {'mobile': mobile}, function (result) {
        ...
        $scope.delModerator = function (moderator) {
            if (confirm('确定删除版主【' + moderator.nickname + '】吗？')) {
                $.ajax({
                    type: 'DELETE',
                    url: 'admin/softCategory/' + $scope.c.id + '/moderators/' + moderator.userId,
