define(['app', 'directive/ng-file'], function (app) {
    ...
    app.controller('vodCategoryCtrl', ['$scope', '$http', function ($scope, $http) {
        $http.get('i/vod_categories').success(function (data) {
            $scope.data = data;
            if (data && data.length) {
                $scope.cg1 = data[0];
            }
        ...
        $scope.delete = function (c) {
            if (c.id) {
                if (confirm('删除类型将导致该类型下所有的文档处于无类型状态，确认删除吗？')) {
                    $http.delete('admin/vodCategory/' + c.id).success(function (data) {
                        if (c.fid) {
