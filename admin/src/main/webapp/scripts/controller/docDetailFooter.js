define(['app', 'umeditor'], function (app, UM) {
    ...
    app.controller('docDetailFooterCtrl', ['$scope', '$http', '$sce', function ($scope, $http, $sce) {
        ...
        editor.ready(function () {
            $http.get('admin/setting?code=CNC_WX_DOC_DETAIL_FOOTER').success(function (s) {
                if (s && s.id) {
                    $scope.entity = s;
        ...
        $scope.save = function () {
            if (!$scope.entity) {
                $scope.entity = {'name': 'CNC_WX_DOC_DETAIL_FOOTER', 'lable': '微信文档详情底部配置'};
            }
