function mBindAccount() {

    this.sendMsg = function (obj_name) {
        if (!OBJ_formcheck.checkFormatM('mobileBind',obj_name, 'phone', '手机号码格式不正确'))
            return false;
        $('#mobileBind').show();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                send_interval = setInterval("OBJ_mBindAccount.changeTimes($('#bind_sendmsg_time_text'),'this.overCallback')", 1000);
            },
            error: function (req) {
                if (req.responseText) {
                    var data = JSON.parse(req.responseText);
                    $('#bindMgs').show();
                    $('#bindMgs_tips').val(data.message).addClass('color-red');
                } else {
                    $('#bindMgs_tips').val('短信发送失败').addClass('color-red');
                }
            }
        });
    ...
    this.bind = function () {
        ...
        var code = $('#code').val();
        if (!OBJ_formcheck.checkFormatM('mobileBind','bindMgs','phone','手机号码格式不对')) {
            return false;
        }
        if (!OBJ_formcheck.checkFormatM('code','bindMgs','code','请输入有效的验证码'))
            return false;
        if (!OBJ_formcheck.checkFormatM('password','bindMgs','password','密码为空或不符合规则')) {
            return false;
        }
        $.ajax({
            ...
            success: function (data) {
                console.log(data);
                $('#bindMgs').show();
                $('#bindMgs_tips').removeClass('color-red').addClass('color-green').val(data);
            ...
            error: function (req) {
                if (req.responseText) {
                    ...
                    $('#bindMgs').show();
                    $('#bindMgs_tips').addClass('color-red').val(err.message);
                } else {
                    console.log("失败。")
                }
            }
...
var OBJ_mBindAccount = new mBindAccount();
var send_interval;